...
class Agent:
    ...
    async def execute_instruction(self, instruction: AgentInstruction, pipeline_context: Dict[str, Any]) -> AgentResponse:
            ...
            if isinstance(value, dict) and "$from_context" in value:
                ...
                try:
                    for path_part in context_path:
                        if isinstance(current_context_value, dict) and path_part in current_context_value:
                            current_context_value = current_context_value[path_part]
                        elif isinstance(current_context_value, list) and path_part.isdigit() and int(path_part) < len(current_context_value):
                            current_context_value = current_context_value[int(path_part)]
                        else:
                            raise KeyError(f"Path part '{path_part}' not found in context for {value['$from_context']}")
        ...
        try:
            result_data = await self._process_command(instruction.command, resolved_parameters, pipeline_context)
...
class RetrievalAgent(Agent):
    ...
    async def _process_command(self, command: str, parameters: Dict[str, Any], pipeline_context: Dict[str, Any]) -> Dict[str, Any]:
        if command == "RETRIEVE_DICOM_STUDY":
            ...
            retrieval_info = await asyncio.to_thread(self.dicom_sniffer.discover_and_retrieve_studies, patient_id, study_uid)
...
class ImageProcessingAgent(Agent):
    ...
    async def _process_command(self, command: str, parameters: Dict[str, Any], pipeline_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if command == "PERFORM_BASIC_SEGMENTATION":
            ...
            segmentation_result = await asyncio.to_thread(self.basic_segmentor.segment_image_volume, dicom_volume, threshold)
            return {"segmentation_result": segmentation_result.model_dump()}
        elif command == "PERFORM_DL_SEGMENTATION":
            dicom_volume_data = parameters["dicom_volume"]
            dicom_volume = DICOMVolume.model_validate(dicom_volume_data)
            segmentation_result = await asyncio.to_thread(self.dl_segmentor.segment_image_volume, dicom_volume)
            return {"segmentation_result": segmentation_result.model_dump()}
        elif command == "RUN_TOTALSEGMENTATOR":
            dicom_volume_data = parameters["dicom_volume"]
            dicom_volume = DICOMVolume.model_validate(dicom_volume_data)
            tasks = parameters.get("tasks")
            segmentation_result = await asyncio.to_thread(self.total_segmentator.run_segmentation, dicom_volume, tasks)
            return {"segmentation_result": segmentation_result.model_dump()}
        else:
...
class SemanticIndexingAgent(Agent):
    ...
    async def _process_command(self, command: str, parameters: Dict[str, Any], pipeline_context: Dict[str, Any]) -> Dict[str, Any]:
        if command == "INDEX_DOCUMENT":
            ...
            await asyncio.to_thread(self.semantic_indexer.index_document, doc_id, text_content, metadata)
            return {"status": "indexed", "doc_id": doc_id}
        elif command == "QUERY_INDEX":
            query_text = parameters["query_text"]
            top_k = parameters.get("top_k", 5)
            results = await asyncio.to_thread(self.semantic_indexer.query_semantic_index, query_text, top_k)
...
class ReportingAgent(Agent):
    ...
    async def _process_command(self, command: str, parameters: Dict[str, Any], pipeline_context: Dict[str, Any]) -> Dict[str, Any]:
        if command == "GENERATE_DIAGNOSTIC_REPORT":
            ...
            diagnostic_findings = [DiagnosticFinding.model_validate(f) for f in parameters["diagnostic_findings"]]
            image_analysis_summaries = [ImageAnalysisResult.model_validate(i) for i in parameters["image_summaries_json"]]
            llm_analysis_summaries = [LLMAnalysisResult.model_validate(l) for l in parameters["llm_summaries_json"]]
            ...
            report_context = {
                ...
                "findings_list_json": json.dumps([f.model_dump() for f in diagnostic_findings]),
                "image_summaries_json": json.dumps([i.model_dump() for i in image_analysis_summaries]),
                "llm_summaries_json": json.dumps([l.model_dump() for l in llm_analysis_summaries]),
            }
            ...
            try:
                ...
                self.logger.error(f"LLM did not return valid JSON for report: {e}. Falling back to basic report.")
                final_report = DiagnosticReport(
                    patient_info=patient_info,
                    study_info=study_info,
                ...
                if response.status == "SUCCESS":
                    ...
                    for key, value in response.output_data.items():
                        if key == "retrieval_info":
                            ...
                            pipeline_context[key] = RetrievedDataInfo.model_validate(value)
                        elif key == "segmentation_result":
                            from msk_io.schema.image_analysis import ImageSegmentationResult
                            pipeline_context[key] = ImageSegmentationResult.model_validate(value)
                        elif key == "llm_analysis_result":
                            from msk_io.schema.llm_output import LLMAnalysisResult
                            pipeline_context[key] = LLMAnalysisResult.model_validate(value)
                        elif key == "report_summary":
                            from msk_io.schema.reports import DiagnosticReport
                            pipeline_context[key] = DiagnosticReport.model_validate(value)
                        else:
