...
class LLMSettings(BaseSettings):
    openai_api_key: SecretStr = Field(default_factory=lambda: SecretStr(os.getenv("OPENAI_API_KEY", "")), description="OpenAI API key for LLM access.")
    google_api_key: SecretStr = Field(default_factory=lambda: SecretStr(os.getenv("GOOGLE_API_KEY", "")), description="Google Cloud API key for LLM access.")
    default_llm_model: str = Field("gpt-4o", description="Default LLM model to use.")
    llm_timeout_seconds: int = Field(60, description="Timeout for LLM API calls in seconds.")
...
class RetrievalSettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="MSKIO_RETRIEVAL_", case_sensitive=False)
...
class AppSettings(BaseSettings):
    ...
    log_file_path: str = Field(default_factory=lambda: os.path.join(os.getcwd(), "msk_io.log"), description="Path to the application log file.")
...
class AppConfig(BaseSettings):
    ...
    def _ensure_directories_exist(self) -> None:
        directories = [self.app.output_directory, self.app.watch_directory, self.retrieval.data_download_dir, self.image_processing.image_cache_dir]
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"OpenAI API Key (first 5 chars): {config.llm.openai_api_key.get_secret_value()[:5]}...")
